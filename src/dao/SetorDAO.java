...
public class SetorDAO {
    ...
    public ArrayList<Setor> listar() {
        try {
            ...
            while (rs.next()) {
                Integer id = rs.getInt("id_setores");
    ...
    public static Setor buscarPorId(Integer id) {
        try {
            String sql = "SELECT * FROM setores WHERE id_setores = ?;";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1,id);
            ...
            if(rs.next()){
                Integer idSetor = rs.getInt("id_setores");
                String nome = rs.getString("nome_setor");
                String responsavel = rs.getString("responsavel");
    ...
    public Boolean atualizar(Setor setor) {
        try {
            String sql = "UPDATE setores SET nome_setor = ?, responsavel = ? WHERE id_setores = ?;";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setString(1,setor.getNome());
            ps.setString(2,setor.getResponsavel());
    ...
    public Boolean remover(Integer id) {
        try {
            String sql = "DELETE FROM setores WHERE id_setores = ?;";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1,id);
